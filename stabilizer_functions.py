...
def randomStimCircuit(N,T,adj_mat,qubitIdxs, probMeasurement):
    
    # N - qubit overhead
    # T - number of gates to be draw for each qubit in qubitsIdx
    # qubitIdxs - indices of qubits that will receive gates
    
    #initialize circuit
    circuit = stim.Circuit()
    circuit.append_operation("I", range(N)) #apply I to everyone to initialize all the qubit indexes
    measurementList = []
    ...
    for i in range(T):
        ...
        for j in qubitIdxs:
            ...
            if random.random() <= probMeasurement:
                
                gate = 'M'
                
            else:
                
                gate = random.choice(["I" ,"H","S","CNOT"])
            
    
            if gate == "CNOT":
                qubit_B = uf.choose_qubit(adj_mat,j)
                circuit.append_operation(gate, [j,qubit_B])
                
            elif gate == "M":
                
                measurementList.append(j)
                circuit.append_operation(gate, [j])
                
            else:
                
                circuit.append_operation(gate, [j])
                
    return circuit, measurementList
...
def stimTableauConverter(N,tableau):

    tableauBinary =np.zeros([N,2*N])
    
    for idx in range(N):
        
        #gets the column for qubit with index idx
        tableau_idx = tableau[(2+3*N)*(2+idx):(2+3*N)*(2+idx+1)]
        
        #get the values for the z column
        m = tableau_idx[4::3]
        ...
        for i in range(N):
            ...
            if m[i] == 'X':
                ...
                tableauBinary[i,2*idx] = 1
                
            elif m[i] == 'Z':
                
                tableauBinary[i,2*idx+1] = 1
    ...
    #getting the global phases of Stim tableau
    gp = tableau[3*N+6:3*N+6+3*(N-1)+1:3]
    ...
    return tableauBinary.astype(int), gpBinary.astype(int)

def cummulativeEntropyStim(N,s):
    
    #string version of the tableau
    tableau = str(s.current_inverse_tableau()**-1)
    #converting to my notation
    M, gp = stimTableauConverter(N,tableau)
    
    cumulativeEntropy = np.zeros(N+1)
...
def stabilizer_group_X(N):
    ...
    for k in range(2**N-1):
        ...
        circuit = np.zeros([N,3])
        for i in range(N):
            circuit[i,:] = np.array([stab_group_X[k,2*i]*4,i,0])
            
        initialOrtCircuits.append(circuit)
              
    return stab_group_X, gp_group_X, initialOrtCircuits
